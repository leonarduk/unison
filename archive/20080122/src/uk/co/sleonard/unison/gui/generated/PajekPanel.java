...
public class PajekPanel extends javax.swing.JPanel implements Observer {
	...
	public PajekPanel(JFrame frame) {
		pajekHeader = new Vector<String>(Arrays.asList(new String[] {
	...
	private Vector<Vector<String>> getLatestPajekMatrixVector() {
		...
		final List<Message> messages = UNISoNController.getInstance()
				.getMessagesFilter();
		HashMap<String, Message> msgMap = new HashMap<String, Message>();

		// Load ALL messages into map so can get complete referemce
		Vector<Message> allMessages = DataQuery.getMessages(null, null,
				session, null, null, false, null, null);
		...
		for (final ListIterator<Message> msgIter = messages.listIterator(); msgIter
			...
			if ((null != refMsgs) && (size > 0)) {
				...
				if (this.allRadio.isSelected()) {
					...
					while (iter.hasNext()) {
						Message msg = getReferencedMessage(msgMap, iter.next(),
								next);
						tableData.add(this.createNewRow(rowIndex++, next, msg));
					}
				} else {
					if (this.previousRadio.isSelected()) {
						lastMsg = getReferencedMessage(msgMap, refMsgs
								.get(refMsgs.size() - 1), next);
					} else if (this.creatorRadio.isSelected()) {
						lastMsg = getReferencedMessage(msgMap, refMsgs.get(0),
								next);
					}
	...
	public Message getReferencedMessage(HashMap<String, Message> msgMap,
			String key, Message currentMessage) {
		...
		if (null != key && key.contains("@") && null == message) {
			...
			UsenetUser poster = new UsenetUser("MISSING", key, "UNKNOWN", null,
					null);
			message = new Message(null, key, currentMessage.getSubject(),
					poster, currentMessage.getTopic(), null, currentMessage
							.getReferencedMessages().replaceAll(key, ""), null);
		}
		return message;
	}
	...
	private void initComponents() {
		...
		saveButton
				.setToolTipText("Save to Pajek Network file (as in Pajek File Preview tab)");
		...
		csvButton
				.setToolTipText("Save to Comma-separated  file (as in Matrixtab).  This can be read into Excel.");
		...
		layout
				.linkSize(javax.swing.SwingConstants.HORIZONTAL,
						new java.awt.Component[] { csvButton, previewButton,
								saveButton });
	...
	private void saveButtonActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_saveButtonActionPerformed
		FileDialog file = new FileDialog(frame, "Save Pajek Network File",
				FileDialog.SAVE);

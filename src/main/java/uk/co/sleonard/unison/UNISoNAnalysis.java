...
public class UNISoNAnalysis {
  ...
  UNISoNAnalysis(
      final NewsGroupFilter filter, final Session session, final HibernateHelper helper) {
    this.filter = filter;
  ...
  public List<ResultRow> getTopCountriesList() {
    ...
    for (final ListIterator<Message> iter = this.filter.getMessagesFilter().listIterator();
        iter.hasNext(); ) {
  ...
  public List<ResultRow> getTopGroupsList() {
    ...
    for (final Entry<NewsGroup, Integer> entry : summaryMap.entrySet()) {
      results.add(new ResultRow(entry.getKey(), entry.getValue().intValue(), NewsGroup.class));
    }
    Collections.sort(results);
    return results;
  }
  ...
  public Vector<Vector<Object>> getTopGroupsVector() throws HibernateException {
    ...
    final String sql =
        "SELECT count(*) as posts, newsgroup_id FROM newsgroup_message "
    ...
    while (iter.hasNext()) {
      ...
      final List<NewsGroup> posters =
          this.helper.runQuery(
  ...
  public Vector<ResultRow> getTopPosters() {
    ...
    for (final Entry<UsenetUser, Integer> entry : summaryMap.entrySet()) {
      results.add(new ResultRow(entry.getKey(), entry.getValue().intValue(), UsenetUser.class));
    }
    Collections.sort(results);
    return results;
  }

...
public class UNISoNController {
  ...
  public UNISoNController(
    ...
    try {
      ...
      this.database =
          new UNISoNDatabase(
              this.filter, hibernateSession, this.helper, new DataQuery(this.helper));
  ...
  public void download(
      final StatusMonitor monitor,
      final NewsGroup[] items,
      final String fromDateString,
      final String toDateString,
      final boolean locationSelected,
      final boolean getTextSelected) {
    if (monitor == null) {
      return;
    }
  ...
  public LinkedBlockingQueue<NewsArticle> getQueue() {
    DataHibernatorWorker.startHibernators(
        this.getNntpReader(), this.helper, this.messageQueue, this.session);
   ...
   */
  public Set<NewsGroup> listNewsgroups(
      final String searchString, final String host, final NewsClient client2)
      throws UNISoNException {
  ...
  public void quickDownload(
      final Set<NewsGroup> groups,
      final Date fromDate1,
      final Date toDate1,
      final DownloadMode mode)
      throws UNISoNException {
    log.debug(
        "Starting quick download of {} groups (mode={}, from={}, to={})",
        groups.size(),
        mode,
        fromDate1,
        toDate1);
  ...
  public void requestDownload(
    ...
    if (null != group) {
      try {
        ...
        log.debug(
            "listNewsgroups returned {} groups",
            (availableGroups2 == null) ? 0 : availableGroups2.size());
        ...
        log.debug(
            "Download request flow complete; monitor enabled = {}", availableGroups2.size() > 0);
      } catch (final UNISoNException e) {
  ...
  private void setButtonState(
      final boolean connectButtonState,
      final boolean downloadButtonState,
      final boolean pauseButtonState,
  ...
  public void setNntpHost(final String nntpHost) {
    ...
    this.headerDownloader =
        new HeaderDownloadWorker(
            this.messageQueue,
            new DownloaderImpl(
                this.nntpHost,
                this.messageQueue,

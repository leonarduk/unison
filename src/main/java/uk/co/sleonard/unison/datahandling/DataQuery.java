...
public class DataQuery {
   ...
   *
   * @param sqlBuffer the sql buffer
   * @param whereClauses the where clauses
   * @return the string buffer
   */
  private StringBuffer addWhereClause(
      final StringBuffer sqlBuffer, final Vector<String> whereClauses) {
  ...
  public Vector<Message> getMessages(
    ...
    if (filtered) {
      ...
      if ((null != users) && (users.size() > 0)) {
        ...
        whereClauses.add(" usenetuser.id in ( " + this.getUsenetUserIdsString(users) + ") ");
      }
      ...
      if ((null != newsgroups) && (newsgroups.size() > 0)) {
        ...
        whereClauses.add(" newsgroup.id in ( " + this.getNewsGroupIdsString(newsgroups) + ") ");
      }
      ...
      if (null != fromDate) {
        whereClauses.add(
            " message.DateCreated >= '" + this.yyyyMMDDFormatter.format(fromDate) + "'");
      }
      ...
      if (null != toDate) {
        ...
        whereClauses.add(
            " message.DateCreated < '" + this.yyyyMMDDFormatter.format(cal.getTime()) + "'");
      }

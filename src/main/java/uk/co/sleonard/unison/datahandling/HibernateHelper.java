...
public class HibernateHelper {
  ...
  private static final String DB_URL = "jdbc:hsqldb:file:src/main/resources/DB/projectDB";
  ...
  public static final String GUI_ARGS[] = {
    "-driver",
    HibernateHelper.dbDriver,
    "-url",
    HibernateHelper.DB_URL,
    "-user",
  ...
  synchronized NewsGroup findOrCreateNewsGroup(final Session session, final String groupName) {
    ...
    for (final String namePart : nameparts) {
      ...
      if (null == group) {
        ...
        group =
            new NewsGroup(namePart, lastGroup, topics, messages, -1, -1, -1, -1, fullName, false);
   ...
   */
  <T> Vector<ResultRow> getListResults(
      final String query, final Class<T> type, final Session session) {
  ...
  public NewsGroup getNewsgroupByFullName(final String groupName, final Session session) {
    final Query query =
        session.createQuery("from " + NewsGroup.class.getName() + " where fullname=?");
  ...
  public synchronized boolean hibernateData(final NewsArticle article, final Session session) {
    ...
    try {
      ...
      final UsenetUser poster =
          this.userFactory.createUsenetUser(article, session, location, gender, this);
   ...
   * @return the vector
   */
  public <T> Vector<T> runQuery(
      final String query, final Session hibernateSession, final Class<T> type) {
   ...
   * @return the list
   */
  private <T> List<T> runSQLQuery(final String query, final Session session, final Class<T> type) {
    log.debug("runSQL: " + query);
    return this.runQuery(session.createNativeQuery(query, type));
  }

  private synchronized void storeNewsgroups(
      final List<String> newsgroupsList, final Message messageInput, final Session session)
      throws HibernateException {
  ...
  public List<NewsGroup> storeNewsgroups(final Set<NewsGroup> newsgroupsList, final Session session)
      throws HibernateException {

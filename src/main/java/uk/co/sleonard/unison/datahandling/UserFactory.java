...
/** Factory responsible for creating {@link UsenetUser} entities and related objects. */
public class UserFactory {
  ...
  public UsenetUser createUsenetUser(
  ...
  private synchronized Location createLocation(
      final NewsArticle article, final Session session, final HibernateHelper helper) {
    Location location;
  ...
  private synchronized IpAddress findOrCreateIpAddress(
      final NewsArticle article, final Session session, final HibernateHelper helper) {
    IpAddress ip = (IpAddress) helper.findByKey(article.getPostingHost(), session, IpAddress.class);
    if (null == ip) {
  ...
  synchronized Location findOrCreateLocation(
      final Session session, final IpAddress ipAddress, final HibernateHelper helper) {
    Location location = this.locationFinder.createLocation(ipAddress.getIpAddress());
    final Location dbLocation =
        (Location) helper.findByKey(location.getCity(), session, Location.class);
    if (null != dbLocation) {
      location = dbLocation;
  ...
  synchronized UsenetUser findUsenetUser(
      final EmailAddress emailAddress, final Session session, final HibernateHelper helper) {
    if ((emailAddress == null) || (emailAddress.getEmail() == null)) {
      return null;
    }

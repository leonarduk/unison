...
public class DownloadNewsPanel extends javax.swing.JPanel
    ...
    java.awt.EventQueue.invokeLater(
        () -> {
          ...
          try {
            ...
            final DownloadNewsPanel panel = new DownloadNewsPanel(UNISoNController.getInstance());
            frame.add(panel);
  ...
  private void availableNewsgroupsValueChanged(
      final javax.swing.event.ListSelectionEvent
          evt) { // GEN-FIRST:event_availableNewsgroupsValueChanged
  ...
  private void downloadButtonActionPerformed(
    ...
    final NewsGroup[] selected =
        this.availableNewsgroups.getSelectedValuesList().toArray(new NewsGroup[0]);
    try {
      this.controller.download(
          this,
          selected,
          this.fromDateField.getText(),
          this.toDateField.getText(),
          this.getLocationCheck.isSelected(),
          false);
    } catch (final Exception e) {
  ...
  private void findButtonActionPerformed(
      final java.awt.event.ActionEvent evt) { // GEN-FIRST:event_findButtonActionPerformed
    this.controller.requestDownload(
        this.newsgroupField.getText(),
        this.hostCombo.getSelectedItem().toString().trim(),
        this,
  ...
  private void initComponents() {
    ...
    this.newsgroupLabel.setToolTipText(
        "Type in full or part name (use * for wild character, e.g. *senior* for all groups with"
            + " \"senior\" in the name)");
    ...
    this.findButton.setToolTipText(
        "Enter name or part name (* for wild chararcter) the n click here");
    this.findButton.addActionListener(evt -> DownloadNewsPanel.this.findButtonActionPerformed(evt));
    ...
    this.pauseButton.addActionListener(
        evt -> DownloadNewsPanel.this.pauseButtonActionPerformed(evt));

    this.cancelButton.setText("Cancel");
    this.cancelButton.addActionListener(
        evt -> DownloadNewsPanel.this.cancelButtonActionPerformed(evt));
    ...
    this.hostLabel.setToolTipText(
        "Look at http://freeusenetnews.com/newspage.html?sortby=articles for other hosts if these"
            + " are broken. Can enter name here.");
    ...
    this.availableNewsgroups.setToolTipText(
        "The number in brackets is an estimate (provided by the server) which is likely to be"
            + " higher than actual number of messages as many messages are deleted.");
  ...
  private void pauseButtonActionPerformed(
      final java.awt.event.ActionEvent evt) { // GEN-FIRST:event_pauseButtonActionPerformed
  ...
  public void updateAvailableGroups(final Set<NewsGroup> availableGroups2) {
    this.availableNewsgroups.setModel(this.controller.getAvailableGroupsModel(availableGroups2));
  }
}

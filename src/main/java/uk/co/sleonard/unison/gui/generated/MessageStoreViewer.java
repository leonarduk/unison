...
class MessageStoreViewer extends javax.swing.JPanel implements DataChangeListener {
  ...
  private TreeNode addChildNode(
      final TreeNode root, final Object childObject, final String nameInput) {
   ...
   * @return the sets the
   */
  private Set<Message> createMessageHierarchy(
      final Set<Message> set,
      final TreeNode root,
   ...
   * @param evt the evt
   */
  private void crosspostComboBoxActionPerformed(
      final java.awt.event.ActionEvent evt) { // GEN-FIRST:event_crosspostComboBoxActionPerformed
   ...
   * @param evt the evt
   */
  private void filterToggleActionPerformed(
      final java.awt.event.ActionEvent evt) { // GEN-FIRST:event_filterToggleActionPerformed
    try {
      this.switchFilter(this.filterToggle.isSelected());
    } catch (final UNISoNException e) {
      if (this.controller.getGui() != null) {
   ...
   */
  private void getBodyButtonActionPerformed(
      final java.awt.event.ActionEvent evt) { // GEN-FIRST:event_getBodyButtonActionPerformed
  ...
  private void groupsHierarchyValueChanged(
      final javax.swing.event.TreeSelectionEvent
          evt) { // GEN-FIRST:event_groupsHierarchyValueChanged
    ...
    if (root.getUserObject() instanceof NewsGroup) {
      ...
      UNISoNController.getInstance().getFilter().setSelectedNewsgroup(newsGroup.getName());
    } else {
   ...
   * @param evt the evt
   */
  private void headersButtonActionPerformed(
      final java.awt.event.ActionEvent evt) { // GEN-FIRST:event_headersButtonActionPerformed
    try {
      ...
      for (final Message message : instance.getFilter().getMessagesFilter()) {
        ...
        if (null == message.getPoster().getLocation()) {
          ...
          FullDownloadWorker.addDownloadRequest(
              message.getUsenetMessageID(),
              DownloadMode.HEADERS,
              nntpHost,
              instance.getQueue(),
              new NewsClientImpl(),
              instance.getNntpReader(),
              instance.getHelper());
        }
      }
    } catch (final UNISoNException e) {
  ...
  private void initComponents() {
    ...
    this.missingMessagesCheck.setBorder(javax.swing.BorderFactory.createEmptyBorder(0, 0, 0, 0));
    ...
    this.filterToggle.addActionListener(
        evt -> MessageStoreViewer.this.filterToggleActionPerformed(evt));
                ...
                layout
                    ...
                    .addGroup(
                        layout
                            ...
                            .addComponent(
                                this.topicsScrollPane,
                                javax.swing.GroupLayout.DEFAULT_SIZE,
                                179,
                                Short.MAX_VALUE)
                            .addComponent(
                                this.groupsScrollPane,
                                javax.swing.GroupLayout.DEFAULT_SIZE,
                                179,
                                Short.MAX_VALUE))
    ...
    layout.linkSize(
        javax.swing.SwingConstants.HORIZONTAL,
        new java.awt.Component[] {this.bodyScrollPane, this.statsTabPane, this.topicsScrollPane});
  ...
  private void missingMessagesCheckItemStateChanged(
      final java.awt.event.ItemEvent evt) { // GEN-FIRST:event_missingMessagesCheckItemStateChanged
    // TODO add your handling code here:
  ...
  private void refreshButtonActionPerformed(
      final java.awt.event.ActionEvent evt) { // GEN-FIRST:event_refreshButtonActionPerformed
    final UNISoNController controller = UNISoNController.getInstance();
  ...
  public void refreshMessagePane() {
    ...
    if (null != message) {
      ...
      final DefaultComboBoxModel<NewsGroup> aModel =
          new DefaultComboBoxModel<>(new Vector<>(message.getNewsgroups()));
  ...
  private void refreshTopCountries() {
    final List<ResultRow> results =
        UNISoNController.getInstance().getAnalysis().getTopCountriesList();
  ...
  private void refreshTopGroups() {
    final List<ResultRow> results = UNISoNController.getInstance().getAnalysis().getTopGroupsList();
  ...
  private void refreshTopPosters() {
    final Vector<ResultRow> results = UNISoNController.getInstance().getAnalysis().getTopPosters();
  ...
  private void switchFilter(final boolean on) throws UNISoNException {
    try {
      ...
      if (on) {
        ...
        final List<GUIItem<ResultRow>> selectedCountries =
            this.topCountriesList.getSelectedValuesList();
        ...
        final List<GUIItem<ResultRow>> selectedNewsgroups =
            this.topGroupsList.getSelectedValuesList();
        ...
        final List<GUIItem<ResultRow>> selectedPosters =
            this.topPostersList.getSelectedValuesList();
        ...
        this.filterToggle.setText("Filter");
        this.filterToggle.setToolTipText(
            "Enter date values, select groups or posters in lists, or combination then click"
                + " filter");
  ...
  private void topCountriesListValueChanged(
      final javax.swing.event.ListSelectionEvent
          evt) { // GEN-FIRST:event_topCountriesListValueChanged
    // TODO add your handling code here:
  ...
  private void topGroupsListValueChanged(
      final javax.swing.event.ListSelectionEvent evt) { // GEN-FIRST:event_topGroupsListValueChanged
  ...
  private void topicsHierarchyValueChanged(
      final javax.swing.event.TreeSelectionEvent
          evt) { // GEN-FIRST:event_topicsHierarchyValueChanged
    final TreePath tp = evt.getPath();
    final TreeNode root = (TreeNode) tp.getLastPathComponent();
  ...
  private void topPostersListValueChanged(
      final javax.swing.event.ListSelectionEvent
          evt) { // GEN-FIRST:event_topPostersListValueChanged
    // TODO add your handling code here:

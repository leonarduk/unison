...
class PajekPanel extends javax.swing.JPanel implements DataChangeListener {
  ...
  PajekPanel(final JFrame frame) {
    this.pajekHeader = new Vector<>(Arrays.asList(new String[] {"Subject", "Date", "FROM", "TO"}));
    try {
   ...
   * @param currentMsg the current msg
   * @param originalMsg the original msg
   * @return the vector
   */
  private Vector<String> createNewRow(
      final int rowIndex, final Message currentMsg, final Message originalMsg) {
    String toText = "";
  ...
  private void csvButtonActionPerformed(
      final java.awt.event.ActionEvent evt) { // GEN-FIRST:event_csvButtonActionPerformed
    try {
  ...
  private Vector<Vector<String>> getLatestPajekMatrixVector() {
    ...
    final List<Message> messages = UNISoNController.getInstance().getFilter().getMessagesFilter();
    ...
    final Vector<Message> allMessages =
        new DataQuery(helper).getMessages(null, null, this.session, null, null, false, null, null);
   ...
   *
   * @param msgMap the msg map
   * @param key the key
   * @param currentMessage the current message
   * @return the referenced message
   */
  private Message getReferencedMessage(
      final HashMap<String, Message> msgMap, final String key, final Message currentMessage) {
    ...
    if ((null != key) && key.contains("@") && (null == message)) {
      ...
      message =
          new Message(
              ...
              poster,
              currentMessage.getTopic(),
              null,
              currentMessage.getReferencedMessages().replaceAll(key, ""),
              null);
    }
    return message;
  }
  ...
  private void initComponents() {
    ...
    this.creatorRadio.setToolTipText(
        "Thsi takes the details of the message's poster along with that of the person who started"
            + " the thread.");
    ...
    this.previousRadio.setToolTipText(
        "This takes the current poster and that of the last message. i.e. the one they actually"
            + " replied to.");
    ...
    this.previewButton.addActionListener(evt -> PajekPanel.this.previewButtonActionPerformed(evt));
    ...
    this.resultsMatrixTable.setModel(
    ...
    layout.setHorizontalGroup(
        layout
            ...
            .addComponent(
                this.pajekTabPane, javax.swing.GroupLayout.DEFAULT_SIZE, 478, Short.MAX_VALUE));
  ...
  private void previewButtonActionPerformed(
      final java.awt.event.ActionEvent evt) { // GEN-FIRST:event_previewButtonActionPerformed
  ...
  private void saveButtonActionPerformed(
      final java.awt.event.ActionEvent evt) { // GEN-FIRST:event_saveButtonActionPerformed
    final FileDialog file = new FileDialog(this.frame, "Save Pajek Network File", FileDialog.SAVE);
    final String initialValue = "*" + PajekPanel.PAJEK_NETWORK_FILE_SUFFIX;
    file.setFile(initialValue); // set initial filename filter
    file.setFilenameFilter(
        (dir, name) -> {

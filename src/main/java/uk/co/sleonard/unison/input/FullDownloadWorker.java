...
public class FullDownloadWorker extends SwingWorker {
  ...
  private static final ArrayList<FullDownloadWorker> downloaders = new ArrayList<>();
   ...
   * @param helper
   * @param session
   * @throws UNISoNException the UNI so n exception @Deprecated move to DownloaderImpl if we can
   */
  public static synchronized void addDownloadRequest(
      final String usenetID,
      final DownloadMode mode,
      final String nntpHost,
      final LinkedBlockingQueue<NewsArticle> queue,
      final NewsClient newsClient,
      final NewsGroupReader nntpReader,
   ...
   */
  private static void startDownloaders(
  ...
  FullDownloadWorker(
      final String server,
      final LinkedBlockingQueue<NewsArticle> outQueue,
      final NewsClient newsClient)
      throws UNISoNException {
    super("FullDownload");
  ...
  NewsArticle convertHeaderStringToArticle(final String theInfo) throws UNISoNException {
    ...
    final NewsArticle article =
        new NewsArticle(
            headerFields.get("MESSAGE-ID"),
            messageNumber,
            date,
            headerFields.get("FROM"),
            headerFields.get("SUBJECT"),
            references,
            bodyBuffer.toString(),
            headerFields.get("NEWSGROUPS"),
  ...
  private boolean storeNextMessage(final LinkedBlockingQueue<NewsArticle> queue)
    ...
    if (null != article) {
      log.debug(
          "Retrieved article {} - subject '{}' queue size before add {}",
          request.getUsenetID(),
          article.getSubject(),
          queue.size());
      log.info(
      ...
      queue.add(article);
      log.debug(
          "Queue size now {} remaining requests {}",
          queue.size(),
          FullDownloadWorker.downloadQueue.size());
      return true;
    }

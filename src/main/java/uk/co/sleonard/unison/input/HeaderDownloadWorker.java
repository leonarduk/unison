...
public class HeaderDownloadWorker extends SwingWorker {
  ...
  public HeaderDownloadWorker(
      final LinkedBlockingQueue<NewsArticle> inputQueue, final Downloader downloader1) {
  ...
  public void initialise(
    ...
    log.info(
        "Initialising: server={}, group={}, start={}, end={}, mode={}, from={}, to={}",
        server,
        newsgroup1,
        startIndex1,
        endIndex1,
    ...
    try {
      ...
      log.warn(
          "Initialisation failed for group {} on {}: {}", newsgroup1, server, e.getMessage(), e);
      throw new UNISoNException("Failed to initialise downloader", e);
    }

    this.downloading = true;
    log.info(
        "Creating " + this.getClass() + " " + newsgroup1 + "[" + reader.getMessageCount() + "]");
  }
  ...
  void processMessage(final LinkedBlockingQueue<NewsArticle> queue1, final String line)
    ...
    if (this.inDateRange(this.fromDate, this.toDate, date)) {
      queue1.add(
          new NewsArticle(articleId, articleNumber, date, from, subject, references, references));
      this.kept++;
    ...
    if (reader != null) {
      ...
      for (String line = bufReader.readLine(); line != null; line = bufReader.readLine()) {
        ...
        if (this.logTally == 100) {
          ...
          log.info(
              "Downloaded {} kept {} skipped {} to process: {} [{}]",
              this.index,
              this.kept,
              this.skipped,
              size,
  ...
  boolean storeArticleInfo(final LinkedBlockingQueue<NewsArticle> queue1) throws UNISoNException {
    ...
    try {
      ...
      for (int i = this.startIndex; i < this.endIndex; i += 500) {
        ...
        try (final Reader reader = this.newsReader.client.retrieveArticleInfo(i, batchEndIndex); ) {

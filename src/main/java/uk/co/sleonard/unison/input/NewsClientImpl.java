...
public class NewsClientImpl implements NewsClient {
  ...
  @Override
  public void connect(
      final String server, final int port, final String username, final String password)
    ...
    try {
      ...
      throw new UNISoNException(
          "Connection refused. \n"
              + "Check your settings are correct: server "
              + server
              + " username "
              + username
              + " password "
   ...
   */
  @Override
  public void connectToNewsGroup(final String hostInput, final String newsgroup) throws Exception {
    if (!this.isConnected()) {
  ...
  @Override
  public BufferedReader retrieveArticleInfo(
      final long lowArticleNumber, final long highArticleNumber) throws IOException {

...
public class PajekNetworkFile {
  ...
  Relationship addRelationship(
      final String ownerName, final String targetName, final List<Relationship> links) {
    ...
    Relationship link =
        new Relationship(this.getVerticeIndex(ownerName), this.getVerticeIndex(targetName));
   ...
   * Creates the links.
   *
   * @param nodePairs the node pairs
   * @param links the links
   * @return the list
   */
  private List<uk.co.sleonard.unison.output.Relationship> createLinks(
      final Vector<Vector> nodePairs, final List<Relationship> links) {
  ...
  public GraphPreviewPanel getPreviewPanel() throws UNISoNException {
    final GraphPreviewPanel graphPreviewPanel =
        new GraphPreviewPanel(this.vertices, this.directedLinks);
  ...
  public void writeData(final PrintStream printStream) {
    ...
    if (this.undirectedLinks.size() > 0) {
      ...
      while (linksIter.hasNext()) {
        ...
        printStream.println(link.getOwner() + " " + link.getTarget() + " " + link.getValue());
      }
    }

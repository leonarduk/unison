...
public class HibernateHelperSetLocationTest {
  ...
  public void testSetLocationCalledOnceWhenLocationChanges() throws Exception {
    ...
    Location oldLocation =
        new Location("OldCity", "OldCountry", "OC", false, new ArrayList<>(), new ArrayList<>());
    Location newLocation =
        new Location("NewCity", "NewCountry", "NC", false, new ArrayList<>(), new ArrayList<>());
    Mockito.when(poster.getLocation()).thenReturn(oldLocation);

    NewsArticle article =
        new NewsArticle(
            "id", 1, new Date(), "name@example.com", "subject", "refs", "content", "group", null);

    Method method =
        HibernateHelper.class.getDeclaredMethod(
            "createUsenetUser", NewsArticle.class, Session.class, Location.class, String.class);

...
public class DataHibernatorWorkerTest {
  ...
  public void clearWorkers() throws Exception {
    ...
    ArrayList<DataHibernatorWorker> workers =
        (ArrayList<DataHibernatorWorker>) workersField.get(null);
    workers.clear();
  }
  ...
  public void testInterruptStopsProcessing() throws Exception {
    ...
    Constructor<DataHibernatorWorker> ctor =
        DataHibernatorWorker.class.getDeclaredConstructor(
            NewsGroupReader.class, HibernateHelper.class, LinkedBlockingQueue.class, Session.class);
    ctor.setAccessible(true);
    DataHibernatorWorker worker = ctor.newInstance(reader, helper, queue, session);
    ...
    Assert.assertTrue("Worker should process at least one message before interrupt", processed > 0);
    Assert.assertTrue("Worker should not process all messages after interrupt", processed < 5);
  }
}

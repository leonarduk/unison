 ...
 * {@link org.apache.commons.net.nntp.NNTPClient}. The NNTP class is made
 * public to provide access to various NNTP constants and to make it easier for
 ...
 * response to a command. When that occurs, the NNTP class method encountering
 ...
 * you may check the last reply code and text with
...
public class NNTP extends SocketClient {
	...
	private void __getReply() throws IOException {
		...
		if (_replyString.length() < 3)
			throw new MalformedServerReplyException("Truncated server reply: "
					+ _replyString);
		try {
			// remove any trailing spaces before parsing - S Leonard
			if (_replyString.equalsIgnoreCase("Bad Article Number")) {
				this._replyCode = NNTPReply.NO_SUCH_ARTICLE_NUMBER;
			} else {
				this._replyCode = Integer.parseInt(_replyString.substring(0, 3)
						.trim());
			}
	...
	protected void _connectAction_() throws IOException {
		...
		_reader_ = new BufferedReader(new InputStreamReader(_input_,
				__DEFAULT_ENCODING));
		_writer_ = new BufferedWriter(new OutputStreamWriter(_output_,
				__DEFAULT_ENCODING));
		__getReply();
	 ...
	 * the actual reply text can be accessed by calling
	 * {@link #getReplyString  getReplyString }.
	 * <p>
	 * 
	 * @param command
	...
	public int sendCommand(String command, String args) throws IOException {
		...
		// System.out.println("CMD : " + __commandBuffer);
	 ...
	 * {@link #getReplyString  getReplyString }. Only use this method if you
	 * are implementing your own NNTP client or if you need to fetch a secondary
	 ...
	 *                receiving the server reply.
	 **************************************************************************/
	public int newnews(String newsgroups, String date, String time,
			boolean GMT, String distributions) throws IOException {
		StringBuffer buffer = new StringBuffer();

...
public class NNTPClient extends NNTP {
	...
	private void __parseGroupReply(String reply, NewsgroupInfo info)
		...
		do {
			...
			info
					._setPostingPermission(NewsgroupInfo.UNKNOWN_POSTING_PERMISSION);
			return;
		} while (false);
	...
	private NewsgroupInfo __parseNewsgroupListEntry(String entry) {
		...
		case 'Y':
			result
					._setPostingPermission(NewsgroupInfo.PERMITTED_POSTING_PERMISSION);
			break;
		case 'n':
		case 'N':
			result
					._setPostingPermission(NewsgroupInfo.PROHIBITED_POSTING_PERMISSION);
			break;
		case 'm':
		case 'M':
			result
					._setPostingPermission(NewsgroupInfo.MODERATED_POSTING_PERMISSION);
			break;
		default:
			result
					._setPostingPermission(NewsgroupInfo.UNKNOWN_POSTING_PERMISSION);
	...
	private Reader __retrieve(int command, int articleNumber,
		...
		int serverReplyCode = sendCommand(command, Integer
				.toString(articleNumber));
		if (!NNTPReply.isPositiveCompletion(serverReplyCode))
			return null;
	 ...
	 * the posting and verify its success or failure from the server reply.
	 * <p>
	 * 
